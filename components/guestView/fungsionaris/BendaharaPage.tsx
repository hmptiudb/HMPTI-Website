...
import { FiDollarSign, FiGithub, FiInstagram, FiMail, FiTrendingUp } from "react-icons/fi";

import { IoCashOutline, IoPeopleCircleOutline, IoSparkles } from "react-icons/io5";
  ...
  useEffect(() => {
    ...
    const fetchMembers = async () => {
      try {
        ...
        /*
         * Hanya mengambil anggota divisi Bendahara
         * dari Firestore.
         *
         * Tidak lagi mengambil seluruh data member
         * lalu memfilternya di browser.
         */
        const membersQuery = query(membersCollection, where("division", "==", "Bendahara"));

        const snapshot = await getDocs(membersQuery);

        const bendaharaMembers = snapshot.docs.map(
          (document) =>
            ({
              id: document.id,
              ...document.data(),
        ...
        /*
         * Koordinator selalu berada di urutan pertama.
         * Anggota lain diurutkan berdasarkan nama.
         */
        const sortedMembers = bendaharaMembers.sort((a, b) => {
        ...
        if (isMounted) {
          setError("Gagal memuat data bendahara");
        }
      } finally {
        if (isMounted) {
          setLoading(false);
...
function LoadingScreen() {
  return (
    <main
      aria-busy="true"
      aria-label="Memuat data divisi Bendahara HMPTI"
      className="relative min-h-screen overflow-hidden bg-white"
    >
      <div aria-hidden="true" className="pointer-events-none absolute inset-0 overflow-hidden">
        <div className="absolute inset-x-0 top-0 h-[420px] bg-gradient-to-br from-green-50/50 via-white to-emerald-50/30" />
        ...
        <div className="absolute -left-32 top-32 hidden h-80 w-80 rounded-full bg-green-100/40 blur-3xl md:block" />
        <div className="absolute -right-24 top-[430px] hidden h-80 w-80 rounded-full bg-emerald-100/30 blur-3xl md:block" />
      </div>
        ...
        <section className="px-4 py-10 sm:px-6 sm:py-14 lg:px-8 lg:py-16">
          <div className="mx-auto max-w-6xl">
            ...
            <div className="mt-10 flex items-center justify-center">
              <div className="inline-flex items-center gap-2 rounded-full border border-green-100 bg-white/85 px-4 py-2 shadow-sm backdrop-blur-sm">
                <span className="h-2 w-2 rounded-full bg-green-500 motion-safe:animate-pulse" />
                <span className="text-xs font-medium text-gray-500 sm:text-sm">Menyiapkan data Bendahara</span>
              </div>
            </div>
          </div>
        </section>
    ...
    >
      ...
      >
        ...
        <motion.p
          ...
          "
        >
          Mengelola keuangan organisasi dengan transparansi dan akuntabilitas tinggi.
        </motion.p>
      </div>
    </section>
  );
      ...
      <div className="mx-auto max-w-6xl">
        ...
        {coordinators.length > 0 && (
          <motion.div
            ...
            "
          >
            <SectionHeading title="Koordinator Bendahara" description="Pemimpin yang mengarahkan dan mengkoordinasi pengelolaan keuangan organisasi." />
        ...
        {regularMembers.length > 0 && (
          ...
          >
            <SectionHeading title="Anggota Bendahara" description="Tim yang menjalankan tugas pengelolaan keuangan dan aset organisasi." />
        ...
        {members.length === 0 && (
          ...
          >
            ...
            <p
              ...
              "
            >
              Data anggota divisi bendahara akan segera tersedia.
            </p>
          </motion.div>
        )}
      </div>
...
function DivisionInfoSection() {
  const responsibilities = [
    "Mengelola arus kas dan anggaran organisasi",
    "Membuat laporan keuangan berkala yang transparan",
  ...
  const skills = [
    ...
    "Ketelitian dan kejujuran dalam bekerja",
    "Kemampuan analisis data keuangan",
    "Komunikasi yang baik dengan donatur dan sponsor",
  ];
        ...
        >
          ...
          >
            ...
            <p
              ...
              "
            >
              Bertanggung jawab atas pengelolaan keuangan dan aset organisasi secara transparan dan akuntabel.
            </p>
          </div>
          ...
          <div
            ...
            "
          >
            <DivisionInfoList icon={<IoSparkles className="text-green-500" />} title="Tanggung Jawab Utama" items={responsibilities} dotClassName="bg-green-500" />

            <DivisionInfoList icon={<FiTrendingUp className="text-emerald-500" />} title="Keahlian yang Dibutuhkan" items={skills} dotClassName="bg-emerald-500" />
          </div>
          ...
          >
            <p
              ...
              "
            >
              &ldquo;Transparansi keuangan adalah pondasi kepercayaan dalam organisasi.&rdquo;
            </p>
          </motion.div>
        </motion.div>
      </div>
