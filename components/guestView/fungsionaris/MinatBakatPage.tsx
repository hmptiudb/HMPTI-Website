...
import { FiGithub, FiInstagram, FiMail, FiHeart, FiAward, FiUsers } from "react-icons/fi";
  ...
  useEffect(() => {
    const fetchMembers = async () => {
      try {
        ...
        // Filter members from Minat & Bakat division
        const minatBakatMembers = allMembers.filter((member) => member.division === "Minat&Bakat");
  ...
  return (
    <main className="min-h-screen bg-white">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-0 left-0 right-0 h-96 bg-gradient-to-br from-pink-50/30 to-rose-50/20"></div>
        ...
        {/* Floating Shapes */}
        <motion.div
          className="absolute top-1/4 left-1/4 w-96 h-96 bg-pink-200/10 rounded-full blur-3xl"
          animate={{
            x: [0, 20, 0],
            y: [0, -20, 0],
          }}
        ...
        />
        <motion.div
          className="absolute bottom-1/3 right-1/4 w-80 h-80 bg-rose-200/10 rounded-full blur-3xl"
          animate={{
            x: [0, -15, 0],
            y: [0, 15, 0],
          }}
  ...
  <div className="min-h-screen flex items-center justify-center bg-white">
    <div className="flex flex-col items-center">
      <motion.div
        ...
        className="w-16 h-16 bg-gradient-to-r from-pink-500 to-rose-500 rounded-full flex items-center justify-center mb-4"
      >
        <IoRibbonOutline className="text-white text-xl" />
      </motion.div>
      <p className="text-gray-600 font-medium">Memuat data minat & bakat...</p>
    </div>
  </div>
);
...
const HeroSection = () => (
  <section className="pt-32 pb-20 px-4 sm:px-6 lg:px-8">
    <div className="max-w-4xl mx-auto text-center">
      ...
      >
        <IoRibbonOutline className="text-pink-500" />
        <span className="text-sm font-medium text-pink-700">Divisi Minat & Bakat</span>
      </motion.div>
      ...
      <div className="max-w-6xl mx-auto">
          ...
          >
            <div className="text-center mb-12">
              <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">
                Koordinator Minat & Bakat
              </h2>
              <p className="text-gray-600 max-w-2xl mx-auto">
                Pemimpin yang menginspirasi dan mengembangkan bakat serta minat anggota organisasi
              </p>
            </div>
        ...
        {regularMembers.length > 0 && (
          ...
          >
            <div className="text-center mb-12">
              ...
              <p className="text-gray-600 max-w-2xl mx-auto">
                Tim yang penuh semangat dalam menggali dan mengembangkan potensi setiap individu
              </p>
            </div>
        ...
        {members.length === 0 && (
          ...
          >
            ...
            <p className="text-gray-500">Data anggota divisi minat & bakat akan segera tersedia</p>
          </motion.div>
        )}
      </div>
    </section>
      ...
      <div className="relative aspect-square overflow-hidden">
        ...
        {isCoordinator && (
          <div className="absolute top-4 left-4">
            <span className="px-3 py-1.5 bg-gradient-to-r from-pink-600 to-rose-500 text-white text-xs font-semibold rounded-full">
              Koordinator
            </span>
          </div>
        )}
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      >
        <div className="text-center mb-12">
          ...
          <h2 className="text-3xl font-bold text-gray-900 mb-4">Tugas Divisi Minat & Bakat</h2>
          <p className="text-gray-600 max-w-2xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            <h3 className="text-xl font-semibold text-gray-900 flex items-center gap-3">
              <FiAward className="text-rose-500" />
        ...
        <div className="mt-12 pt-8 border-t border-gray-100">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { name: "Seni", color: "bg-pink-100 text-pink-700" },
              { name: "Olahraga", color: "bg-blue-100 text-blue-700" },
              { name: "Musik", color: "bg-purple-100 text-purple-700" },
              { name: "Menulis", color: "bg-green-100 text-green-700" },
              { name: "Public Speaking", color: "bg-yellow-100 text-yellow-700" },
              { name: "Leadership", color: "bg-indigo-100 text-indigo-700" },
              { name: "Debat", color: "bg-red-100 text-red-700" },
              { name: "Robotika", color: "bg-gray-100 text-gray-700" },
            ].map((field, index) => (
              <div key={index} className={`px-3 py-2 rounded-lg text-center text-sm font-medium ${field.color}`}>
                {field.name}
              </div>
            ))}
          </div>
        ...
        >
          <p className="text-gray-600 italic">
            "Setiap orang memiliki bakat terpendam, tugas kami adalah membantu menemukan dan mengembangkannya."
          </p>
        </motion.div>
      </motion.div>
    </div>
